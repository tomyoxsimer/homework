...
def uninformedSearch(grid, start, goal):
    ...
    """
    # I could not get Depth first search to work so
    # here is the code I was trying to use to get it
    # to work, commented out so everything else can
    # run normally.

    # def uniformedSearch(grid, start, goal, mode):
    # if mode == 'BFS':
    ...
    while not (current.value is goal):
        ...
        if current.value == goal:
            ...
            print('Number of Nodes Expanded:', numExpanded)
            return path
...
def setPath(current, path, start):
    """
    This function should be called once the goal has been found and the final path must be obtained. Create a function
    called setPath that takes in two parameters: ‘current’ and ‘path’.
    The ‘current’ parameter should be the Node object whose value is the goal location.
    The ‘path’ parameter should be an empty list that gets filled in with the final path by the setPath function.
    """
    pathCost = 0
    x = current
    while x.parent is not None:
        ...
        pathCost += 1
    path.append([start[0], start[1]]) # had to add in start because I was having issues with outputGrid
...
def main():
    ...
    for i in grid:
        print(i)
    print('\n')
    print('Choose your start location. Example Response: 1,1')
    s = input("Choosing any '1' will result in failure: ")
    e = input("Choose your end location: ")
    a = s.split(',')
    b = e.split(',')
    ...
    ### DFS Implementation ###
    # mode = input("Input 'BFS' for breadth first search\n Input 'DFS' for depth first search")
    # path = uninformedSearch(grid, start, goal, mode)
